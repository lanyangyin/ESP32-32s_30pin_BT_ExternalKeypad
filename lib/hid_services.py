...
class Advertiser:
    ...
    def __init__(self, ble, services=[UUID(0x1812)], appearance=const(960), name="Generic HID Device"):
        ...
        print("广播器已创建: ", self.decode_name(self._payload), " 服务: ", self.decode_services(self._payload))
...
class HumanInterfaceDevice(object):
    ...
    DEVICE_STOPPED = const(0)  # 设备未进行广播或连接。
    DEVICE_IDLE = const(1)    # 设备处于空闲状态，可以进行广播。
    DEVICE_ADVERTISING = const(2)  # 设备正在广播。
    DEVICE_CONNECTED = const(3)  # 设备已连接。
    ...
    def __init__(self, device_name="Generic HID Device"):
        ...
        self.model_number = "1"                                                                                         # The 型号编号 characteristic.
        self.serial_number = "1"                                                                                        # The 序列号 characteristic.
        self.firmware_revision = "1"                                                                                    # The 固件版本 characteristic.
        self.hardware_revision = "1"                                                                                    # The 硬件版本 characteristic.
        self.software_revision = "2"                                                                                    # The 软件版本 characteristic.
        self.manufacture_name = "Homebrew"                                                                              # The 制造商名称 characteristic.
        ...
        self.battery_level = 100                                                                                        # The 电池电量 characteristic (percentages).

        # Device Information Service (DIS) description.
        self.DIS = (
            UUID(0x180A),                                                                                               # 0x180A = Device Information.
            (
                (UUID(0x2A24), F_READ),                                                                                 # 0x2A24 = 型号编号 string, to be read by client.
                (UUID(0x2A25), F_READ),                                                                                 # 0x2A25 = 序列号 string, to be read by client.
                (UUID(0x2A26), F_READ),                                                                                 # 0x2A26 = 固件版本 string, to be read by client.
                (UUID(0x2A27), F_READ),                                                                                 # 0x2A27 = 硬件版本 string, to be read by client.
                (UUID(0x2A28), F_READ),                                                                                 # 0x2A28 = 软件版本 string, to be read by client.
                (UUID(0x2A29), F_READ),                                                                                 # 0x2A29 = 制造商名称 string, to be read by client.
        ...
        self.BAS = (
            ...
            (
                (UUID(0x2A19), F_READ_NOTIFY, (                                                                         # 0x2A19 = 电池电量, to be read by client after being notified of change.
    ...
    def ble_irq(self, event, data):
        if event == _IRQ_CENTRAL_CONNECT:                                                                               # 中央设备已连接.
            ...
            print("中央设备已连接:", self.conn_handle)
        elif event == _IRQ_CENTRAL_DISCONNECT:                                                                          # 中央服务器已断开连接.
        ...
        elif event == _IRQ_GATTS_READ_REQUEST:                                                                          # 读取请求 from client.
            conn_handle, attr_handle = data
            description, val = self.characteristics.get(attr_handle, (None, None))
            print("读取请求:", description if description else attr_handle, "with value" if val else "", val if val else "")
            ...
            print("连接更新。间隔=", conn_interval, "延迟=", conn_latency, "超时=", supervision_timeout, "状态=", status)
            ...
            print("更新密钥:", conn_handle, self.encrypted, self.authenticated, self.bonded, self.key_size)
            ...
            print("返回密钥:", bytes(value) if value else None, "for", "key" if key else "index", _key if key else index, "with type", sec_type)
            return value
        else:
            print("未处理的 IRQ 事件:", event)
    ...
    def start(self):
        if self.device_state is HumanInterfaceDevice.DEVICE_STOPPED:
            ...
            print("BLE 已开启，MAC 地址类型为", "随机" if addr_type else "公共", "MAC 地址", addr)
    ...
    def save_service_characteristics(self, handles):
        ...
        print("正在保存设备信息服务特征")
        self.characteristics[h_mod] = ("型号编号", string_pack(self.model_number, 24))
        self.characteristics[h_ser] = ("序列号", string_pack(self.serial_number, 16))
        self.characteristics[h_fwr] = ("固件版本", string_pack(self.firmware_revision, 8))
        self.characteristics[h_hwr] = ("硬件版本", string_pack(self.hardware_revision, 16))
        self.characteristics[h_swr] = ("软件版本", string_pack(self.software_revision, 8))
        self.characteristics[h_man] = ("制造商名称", string_pack(self.manufacture_name, 36))
        self.characteristics[h_pnp] = ("即插即用信息", struct.pack("<BHHH", self.pnp_manufacturer_source, self.pnp_manufacturer_uuid, self.pnp_product_id, self.pnp_product_version))

        print("正在保存电池服务特征")
        self.characteristics[self.h_bat] = ("电池电量",  struct.pack("<B", self.battery_level))
    ...
    # Returns the 电池电量 (percentage).
    def get_battery_level(self):
        return self.battery_level

    # Sets the value for the 电池电量 (percentage).
    ...
    # Notifies the client by writing to the 电池电量 handle.
    def notify_battery_level(self):
        if self.is_connected():
            print("Notify 电池电量: ", self.battery_level)
            value = struct.pack("<B", self.battery_level)
            self.characteristics[self.h_bat] = ("电池电量", value)
...
class Joystick(HumanInterfaceDevice):
    def __init__(self, name="Bluetooth Joystick"):
        ...
        self.HIDS = (                                                                                                   # HID service description: describes the service and how we communicate
            ...
            (
                (UUID(0x2A4A), F_READ),                                                                                 # 0x2A4A = HID 信息 characteristic, to be read by client.
    ...
    def save_service_characteristics(self, handles):
        ...
        self.characteristics[h_info] = ("HID 信息", b"\x01\x01\x00\x02")                                         # HID info: ver=1.1, country=0, flags=normal.
        self.characteristics[h_hid] = ("HID 输入报告 map", self.HID_INPUT_REPORT)                                   # HID 输入报告 map.
    ...
    def notify_hid_report(self):
        if self.is_connected():
            ...
            print("通知并发送报告: ", struct.unpack("bbB", state))
...
class Mouse(HumanInterfaceDevice):
    def __init__(self, name="Bluetooth Mouse"):
        ...
        self.HIDS = (                                                                                                   # Service description: describes the service and how we communicate.
            UUID(0x1812),                                                                                               # 0x1812 = Human Interface Device.
            (
                (UUID(0x2A4A), F_READ),                                                                                 # 0x2A4A = HID 信息, to be read by client.
    ...
    def notify_hid_report(self):
        if self.is_connected():
            ...
            print("通知并发送报告: ", struct.unpack("Bbbb", state))
...
class Keyboard(HumanInterfaceDevice):
    ...
    def save_service_characteristics(self, handles):
        ...
        self.characteristics[h_hid] = ("HID 输入报告 map", self.HID_INPUT_REPORT)                                   # HID 输入报告 map.
        self.characteristics[h_ctrl] = ("HID control point", b"\x00")                                                   # HID control point.
        self.characteristics[self.h_rep] = ("HID 输入报告", state)                                                  # HID report.
        self.characteristics[h_cccd] = ("HID 输入 CCCD", b"\x01")                                                      # HID CCCD: notify=true, indicate=false.
        self.characteristics[h_d1] = ("HID 输入引用", struct.pack("<BB", 1, 1))                                  # HID reference: id=1, type=input.
        self.characteristics[self.h_repout] = ("HID 输出报告", state)                                              # HID report.
        self.characteristics[h_cccd2] = ("HID 输出 CCCD", b"\x00")                                                    # HID CCCD: notify=false, indicate=false.
        self.characteristics[h_d2] = ("HID 输出参考", struct.pack("<BB", 1, 2))                                 # HID reference: id=1, type=output.
        self.characteristics[h_proto] = ("HID 协议模式", b"\x01")                                                  # HID protocol mode: report.
    ...
    def notify_hid_report(self):
        if self.is_connected():
            ...
            state = struct.pack("8B", self.modifiers, 0, self.keypresses[0], self.keypresses[1], self.keypresses[2], self.keypresses[3], self.keypresses[4], self.keypresses[5])
            self.characteristics[self.h_rep] = ("HID 输入报告", state)
            self._ble.gatts_notify(self.conn_handle, self.h_rep, state)                                                 # Notify central by writing to the report handle.
            print("通知并发送报告: ", struct.unpack("8B", state))
